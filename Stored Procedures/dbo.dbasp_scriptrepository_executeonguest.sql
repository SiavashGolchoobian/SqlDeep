...
-- Version:		<3.0.0.0>
-- Description:	<Execute downloaded scripts that archived on Guest DBA.dbo.ScriptRepositoryGuest in sequentialy order>
...
BEGIN
	...
	WHILE EXISTS (	SELECT 1 
					FROM [DBA].[dbo].[ScriptRepositoryGuest] AS myGuest 
	...
	BEGIN
		...
		FROM 
			[DBA].[dbo].[ScriptRepositoryGuest] AS myGuest 
		WHERE	
			[myGuest].[IsEnabled]=1
			AND CASE @myIgnoreCheckValue WHEN 1 THEN 1 ELSE [myGuest].[CalculatedCheckValue] END = CASE @myIgnoreCheckValue WHEN 1 THEN 1 ELSE [myGuest].[CheckValue] END
			AND ISNULL([myGuest].[LastExecutionStatus], N'') NOT IN (@myStatus_SUCCEED,@myStatus_CANCELED)
		...
		--Step 02: Replacing this order with last newer related order
			...
			IF EXISTS (SELECT 1 FROM [DBA].[dbo].[ScriptRepositoryGuest] WHERE [RecordId]>@myRecordId AND [IsEnabled]=1 AND CASE @myIgnoreCheckValue WHEN 1 THEN 1 ELSE [CalculatedCheckValue] END = CASE @myIgnoreCheckValue WHEN 1 THEN 1 ELSE [CheckValue] END AND [RecordRef]=@myRecordId)
			BEGIN
				...
				FROM
					[DBA].[dbo].[ScriptRepositoryGuest] AS mySource
		...
		--Sep03: Retriving order parameters
			...
			BEGIN TRY
				...
				UPDATE [DBA].[dbo].[ScriptRepositoryGuest] SET [LastExecutionStatus]=@myStatus_INPROGRESS,[LastExecutionDate]=GETDATE() WHERE [RecordId]=@myRecordId
				EXECUTE (@myScriptText);
				UPDATE [DBA].[dbo].[ScriptRepositoryGuest] SET [LastExecutionStatus]=@myStatus_SUCCEED WHERE [RecordId]=@myRecordId
