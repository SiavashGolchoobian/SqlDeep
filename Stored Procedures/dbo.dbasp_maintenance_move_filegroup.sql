...
-- Version:		<3.0.0.5>
-- Description:	<Move all tables from @SourceFilegroup to TargetFilegroups>	--TEXTIMAGE source: https://blog.dbi-services.com/moving-tables-online-on-filegroup-with-constraints-and-lob-data/
-- Input Parameters:
...
--	@TargetLobFilegroup:		'LobFG' or '<SameAsDataFilegroup>' Filegroup name for LOB column objects of tables under @SourceFilegroup, if you use '<SameAsDataFilegroup>' this value, LOB filegroup will automaticaly move LOB of the table to same filegroup as IN-ROW data (regular data)
--	@IndexWithOptions:			any extra with options for index creation, maximum switches are (you can modify sp code to force some smart options): N'STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = ON, ONLINE = OFF, DATA_COMPRESSION = PAGE, MAXDOP=0, FillFactor = 90'
--	@ConstraintWithOptions:		any extra with options for constraint creation, maximum switches are (you can modify sp code to force some smart options): N'STATISTICS_NORECOMPUTE = OFF, DATA_COMPRESSION = PAGE, FillFactor = 90'
...
CREATE PROCEDURE [dbo].[dbasp_maintenance_move_filegroup]
(
	...
	@TargetLobFilegroup NVARCHAR(128)=N'<SameAsDataFilegroup>',
	@IndexWithOptions NVARCHAR(1000)=N'STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = ON, ONLINE = OFF, DATA_COMPRESSION = ROW, MAXDOP=0, FillFactor = 90',
	@ConstraintWithOptions NVARCHAR(1000)=N'STATISTICS_NORECOMPUTE = OFF, DATA_COMPRESSION = ROW, FillFactor = 90',
...
BEGIN
	...
	IF UPPER(@TargetLobFilegroup)=UPPER(N'<SameAsDataFilegroup>')
		SET @myUseSameLobFgAsDataFg=1
	...
	END
	--Check @TargetLobFilegroup existence
	IF @myUseSameLobFgAsDataFg=0 AND NOT EXISTS(SELECT 1 FROM sys.filegroups as myFG WHERE myFG.name=@TargetLobFilegroup)
	BEGIN
		SET @myIsPrerequisitesPassed=0
		SET @myMessage=@myMessage + N'Target LOB filegroup (' + @TargetLobFilegroup + N') does not exists.' + @myNewLine
	END
	...
	CREATE TABLE #myTablesList (Row_id BIGINT IDENTITY,[object_id] INT,TableName NVARCHAR(256),index_id INT,index_name NVARCHAR(128),
								...
								OriginalWithOptions NVARCHAR(1000),TableHasFilestream BIT,FilestreamFilegroupName NVARCHAR(128),TableHasLOB BIT)
	...
	SELECT
		...
		OriginalWithOptions=ISNULL(
			CASE WHEN myIndexes.is_padded=1 THEN N'PAD_INDEX = ON,' ELSE N'PAD_INDEX = OFF,' END + 
			CASE WHEN myIndexes.[ignore_dup_key]=1 THEN N'IGNORE_DUP_KEY = ON,' ELSE N'IGNORE_DUP_KEY = OFF,' END + 
			CASE WHEN myIndexes.[allow_row_locks]=1 THEN N'ALLOW_ROW_LOCKS = ON,' ELSE N'ALLOW_ROW_LOCKS = OFF,' END + 
			CASE WHEN myIndexes.[allow_page_locks]=1 THEN N'ALLOW_PAGE_LOCKS = ON,' ELSE N'ALLOW_PAGE_LOCKS = OFF,' END + 
			CASE WHEN myIndexes.fill_factor>0 THEN N'FILLFACTOR = ' + CAST(myIndexes.fill_factor AS NVARCHAR(3)) + N',' ELSE N'' END,N''),
		TableHasFilestream=
	...
	SET @myCursor=CURSOR FOR SELECT Row_id ,[object_id] ,TableName ,index_id ,index_name ,index_type ,is_disabled ,has_filter ,filter_definition ,TableHasPrimaryKey ,TableHasUniqueCnst ,TableHasUniqueIndex ,TableHasIdentity ,TableHasRowGuidCol ,TableHasSpareColumn ,OriginalWithOptions,TableHasFilestream,FilestreamFilegroupName,TableHasLOB FROM #myTablesList
	Open @myCursor
	FETCH NEXT FROM @myCursor INTO @myRow_id ,@myObject_id ,@myTableName ,@myIndex_id ,@myIndex_name ,@myIndex_type ,@myIs_disabled ,@myHas_filter ,@myFilter_definition ,@myTableHasPrimaryKey ,@myTableHasUniqueCnst ,@myTableHasUniqueIndex ,@myTableHasIdentity ,@myTableHasRowGuidCol ,@myTableHasSpareColumn ,@myOriginalWithOptions ,@myTableHasFilestream, @myFilestreamFilegroupName, @myTableHasLOB
	...
	BEGIN
		...
		SET @myTempPartitionSchemeFilegroup=NULL
		SET @myWithOption=
				CASE	--DATA_COMPRESSION feature can not be used with Spare Column enabled tables
					WHEN LEN(@IndexWithOptions)>0 AND @myTableHasSpareColumn=0 THEN @myOriginalWithOptions + N',' + @IndexWithOptions 
					WHEN LEN(@IndexWithOptions)>0 AND @myTableHasSpareColumn=1 THEN @myOriginalWithOptions + N',' + REPLACE(REPLACE(@IndexWithOptions,N'PAGE',N'NONE'),N'ROW',N'NONE')
					ELSE @myOriginalWithOptions
				 END
		...
		SET @myConstraintWithOptions=
				CASE	--DATA_COMPRESSION feature can not be used with Spare Column enabled tables
					WHEN LEN(@ConstraintWithOptions)>0 AND @myTableHasSpareColumn=0 THEN @ConstraintWithOptions
					WHEN LEN(@ConstraintWithOptions)>0 AND @myTableHasSpareColumn=1 THEN REPLACE(REPLACE(@ConstraintWithOptions,N'PAGE',N'NONE'),N'ROW',N'NONE')
					ELSE @ConstraintWithOptions
				 END
		...
		BEGIN	--<Heap Block>
			...
			BEGIN
				...
				BEGIN
					...
					SET @myTempPartitionSchemeFilegroup = CASE @myUseSameLobFgAsDataFg WHEN 1 THEN @TargetHeapFilegroup ELSE @TargetLobFilegroup END
					...
					BEGIN
						SET @mySqlStr=CAST(N'CREATE PARTITION SCHEME ' + CAST(QUOTENAME(@myTempPartitionSchemeNameHeap) AS NVARCHAR(MAX)) + N' AS PARTITION ' + CAST(QUOTENAME(@myTempPartitionFunctionName) AS NVARCHAR(MAX)) + N' ALL TO (' + CAST(QUOTENAME(@myTempPartitionSchemeFilegroup) AS NVARCHAR(MAX)) + N')' AS NVARCHAR(MAX))
						INSERT INTO #myCommandList (ObjectID,TableType, SqlCommand ) VALUES (@myObject_id,@myIndex_type,@mySqlStr)
				...
				IF @myTableHasLOB=1 AND @UsePartitioningForTextImageMovement=1
				BEGIN
					--LOB needs to set Filegroup firstly on a PS, and then set to explicitly corresponding FG 
					SET @mySqlStr=CAST(N'CREATE UNIQUE CLUSTERED INDEX ' + CAST(QUOTENAME(@myTempIndexName) AS nvarchar(MAX)) + N'
										ON ' + CAST(@myTableName AS nvarchar(MAX)) + N'(' + CAST(@myIdentColumnName AS nvarchar(MAX)) + N')
										WITH (DROP_EXISTING = ON' + CASE WHEN LEN(@myWithOption)>0 THEN N',' ELSE N'' END + @myWithOption + N')
										ON ' + CAST(QUOTENAME(@myTempPartitionSchemeFilegroup) AS nvarchar(MAX)) + CASE @myTableHasFilestream WHEN 1 THEN N' FILESTREAM_ON ' + CAST(QUOTENAME(@TargetFilestreamFilegroup) AS nvarchar(MAX)) ELSE N'' END
								AS nvarchar(MAX))
					INSERT INTO #myCommandList (ObjectID, TableType, SqlCommand ) VALUES (@myObject_id,@myIndex_type,@mySqlStr)
					
					--For sending regular data to DataFG
					SET @mySqlStr=CAST(N'CREATE UNIQUE CLUSTERED INDEX ' + CAST(QUOTENAME(@myTempIndexName) AS nvarchar(MAX)) + N'
										ON ' + CAST(@myTableName AS nvarchar(MAX)) + N'(' + CAST(@myIdentColumnName AS nvarchar(MAX)) + N')
										WITH (DROP_EXISTING = ON' + CASE WHEN LEN(@myWithOption)>0 THEN N',' ELSE N'' END + @myWithOption + N')
			...
			BEGIN
				...
				ORDER BY 
					myIndexColumns.[key_ordinal]	--myIndexColumns.index_column_id
						...
						@myTempPartitionColumnType=[myDataType].[name]+
							CASE 
								...
								ELSE
									N''
								END
					FROM sys.index_columns AS myIndexColumns INNER JOIN sys.columns AS myColumns ON myColumns.[object_id] = myIndexColumns.[object_id] AND myColumns.column_id = myIndexColumns.column_id INNER JOIN sys.[types] AS myDataType ON [myColumns].[system_type_id]=[myDataType].[user_type_id] WHERE myIndexColumns.[object_id]=@myObject_id AND myIndexColumns.index_id=@myUQ_index_id ORDER BY [myIndexColumns].[key_ordinal] --myIndexColumns.index_column_id
					SET @myTempPartitionColumnTypeConverted=REPLACE(REPLACE(REPLACE(@myTempPartitionColumnType,N'(',N''),N')',N''),N',',N'o')
					SET @myTempPartitionFunctionName=N'PF' + @myTempIndexName + N'_' + @myTempPartitionColumnTypeConverted
					SET @myTempPartitionSchemeNameHeap=N'PSH' + @myTempIndexName + N'_' + @myTempPartitionColumnTypeConverted
					SET @myTempPartitionSchemeNameFilestream=N'PSF' + @myTempIndexName + N'_' + @myTempPartitionColumnTypeConverted
						...
						@myTempPartitionColumnType=[myDataType].[name]+
							CASE 
								...
								ELSE
									N''
								END
					FROM sys.index_columns AS myIndexColumns INNER JOIN sys.columns AS myColumns ON myColumns.[object_id] = myIndexColumns.[object_id] AND myColumns.column_id = myIndexColumns.column_id INNER JOIN sys.[types] AS myDataType ON [myColumns].[system_type_id]=[myDataType].[user_type_id] WHERE myIndexColumns.[object_id]=@myObject_id AND myIndexColumns.index_id=@myIndex_id ORDER BY [myIndexColumns].[key_ordinal]	--myIndexColumns.index_column_id
					SET @myTempPartitionColumnTypeConverted=REPLACE(REPLACE(REPLACE(@myTempPartitionColumnType,N'(',N''),N')',N''),N',',N'o')
					SET @myTempPartitionFunctionName=N'PF' + @myTempIndexName + N'_' + @myTempPartitionColumnTypeConverted
					SET @myTempPartitionSchemeNameHeap=N'PSH' + @myTempIndexName + N'_' + @myTempPartitionColumnTypeConverted
					SET @myTempPartitionSchemeNameFilestream=N'PSF' + @myTempIndexName + N'_' + @myTempPartitionColumnTypeConverted
			...
			BEGIN
				...
				BEGIN
					...
					IF NOT EXISTS (SELECT 1 FROM #myCommandList AS myCommandList WHERE myCommandList.[SqlCommand] LIKE N'CREATE PARTITION SCHEME \' + QUOTENAME(@myTempPartitionSchemeNameHeap) + N'%' ESCAPE '\')
					BEGIN
						SET @mySqlStr=N'CREATE PARTITION SCHEME ' + QUOTENAME(@myTempPartitionSchemeNameHeap) + N' AS PARTITION ' + QUOTENAME(@myTempPartitionFunctionName) + N' ALL TO (' + QUOTENAME(@myTempPartitionSchemeFilegroup) + N')'
						INSERT INTO #myCommandList (ObjectID,TableType, SqlCommand ) VALUES (@myObject_id,@myIndex_type,@mySqlStr)
		...
		BEGIN	--<Clustered Block>
			...
			BEGIN
				...
				BEGIN
					...
					SET @myTempPartitionSchemeNameFilestream=N'PSF' + @myTempIndexName + N'_' + @myTempPartitionColumnTypeConverted
					SET @myTempPartitionSchemeFilegroup = CASE @myUseSameLobFgAsDataFg WHEN 1 THEN @TargetClusteredFilegroup ELSE @TargetLobFilegroup END
					--Create Corrsponding Partition FUNCTION (if not exists)
					IF NOT EXISTS (SELECT 1 FROM #myCommandList AS myCommandList WHERE myCommandList.[SqlCommand] LIKE N'CREATE PARTITION FUNCTION \' + QUOTENAME(@myTempPartitionFunctionName) + N'%' ESCAPE '\')
					BEGIN
						SET @mySqlStr=CAST(N'CREATE PARTITION FUNCTION ' + CAST(QUOTENAME(@myTempPartitionFunctionName) AS nvarchar(MAX)) + N' (' + CAST(@myTempPartitionColumnType AS nvarchar(MAX)) + N') AS RANGE RIGHT FOR VALUES()' AS nvarchar(MAX))
					...
					BEGIN
						SET @mySqlStr=CAST(N'CREATE PARTITION SCHEME ' + CAST(QUOTENAME(@myTempPartitionSchemeNameClustered) AS nvarchar(MAX)) + N' AS PARTITION ' + CAST(QUOTENAME(@myTempPartitionFunctionName) AS nvarchar(MAX)) + N' ALL TO (' + CAST(QUOTENAME(@myTempPartitionSchemeFilegroup) AS nvarchar(MAX)) + N')' AS nvarchar(MAX))
						INSERT INTO #myCommandList (ObjectID,TableType, SqlCommand ) VALUES (@myObject_id,@myIndex_type,@mySqlStr)
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						--For avoiding existed pk error according to executing next block statement
						SET @mySQLStr=CAST(N'ALTER TABLE ' + CAST(@myTableName AS nvarchar(MAX)) + N' DROP CONSTRAINT ' + CAST(@myIndex_name AS nvarchar(MAX)) AS nvarchar(MAX))
						INSERT INTO #myCommandList (ObjectID, TableType, SqlCommand ) VALUES (@myObject_id,@myIndex_type,@mySqlStr)
...
END
GO
EXEC sp_addextendedproperty N'Author', N'Siavash Golchoobian', 'SCHEMA', N'dbo', 'PROCEDURE', N'dbasp_maintenance_move_filegroup', NULL, NULL
GO
EXEC sp_addextendedproperty N'Created Date', N'2016-03-26', 'SCHEMA', N'dbo', 'PROCEDURE', N'dbasp_maintenance_move_filegroup', NULL, NULL
GO
EXEC sp_addextendedproperty N'Modified Date', N'2018-07-15', 'SCHEMA', N'dbo', 'PROCEDURE', N'dbasp_maintenance_move_filegroup', NULL, NULL
GO
EXEC sp_addextendedproperty N'Version', N'3.0.0.5', 'SCHEMA', N'dbo', 'PROCEDURE', N'dbasp_maintenance_move_filegroup', NULL, NULL
GO
